...
module.exports.updateAmoLinkInDoli = async function updateAmoLinkInDoli(
  callback
) {
...
module.exports.insertAmoConmnection = async function insertAmoConmnection(
  url,
  integrationId,
  secretKey,
  code,
...
module.exports.deleteAmoConnection = async function deleteAmoConnection(
  id,
  callback
) {
  try {
...
module.exports.getAllAmoConnections = async function getAllAmoConnections(
  callback
) {
  amoconectionRepo
    .getAll()
...
// module.exports.addNewContactsToAmo = async function addNewContactsToAmo(
//   users,
//   doliUsers,
//   callback
// ) {
...
//           addContactsToAmo(data.url, data.accessToken, users)
//             .then((data) => {
...
//           addContactsToAmo(data.url, data.accessToken, userSlice)
//             .then((data) => {
...
module.exports.addNewContactsToAmo = async function addNewContactsToAmo(
  ...
  if (rows.length > 0) {
    ...
    if (users.length <= 250) {
      console.log("Addinng last elements to amo");
      const data = await addContactsToAmo(
        connection.url,
        connection.accessToken,
        users
      );
      const ids = data._embedded.contacts.map((value) => value.id);
      addContactsToAmoDB(ids, doliUsers);
      return;
    } else {
      console.log("Addinng 250 elements to amo");
      const userSlice = users.slice(0, 250);
      ...
      const ids = data._embedded.contacts.map((value) => value.id);
      addContactsToAmoDB(ids, doliUserSlice);
      users.splice(0, 250);
      doliUsers.splice(0, 250);
      return addNewContactsToAmo(users, doliUsers);
    }
  }
};
    ...
    while (i < usersToAdd) {
      ...
      try {
        ...
        if (i >= usersToAdd - 1) {
          const dif = new Date() - start;
          logger.info(
            "Finished to add new contacts to doli in ms: " + dif.toString()
          );
          callback(null);
        }
      } catch (e) {
...
*/
module.exports.addNewAmoContact = async function addNewAmoContact(
  contact,
  callback
) {
  let allTags = [];
  ...
  try {
    ...
    if (contact.custom_fields) {
      contact.custom_fields.map((custom) => {
        if (custom.id == 243912) {
          //243912 es el id de Phone)
          contactInfo["Phone"] = custom.values[0].value;
        }
        if (custom.id == 243914) {
          //243914 es el id de Email)
...
module.exports.deleteAmoContact = async function deleteAmoContact(
  contact,
  callback
) {
...
module.exports.updateAmoContact = async function updateAmoContact(
  contact,
  callback
) {
  try {
  ...
  amoRepo.getByID(ID).then((amoUsers) => {
    ...
    if (amoUsers.length > 0) {
      ...
      if (user.DoliID) {
        if (user.DoliID > 0) {
          try {
            ...
            editContactInDoli(URLDoli, TOKENDoli, contactInfo, user.DoliID, 3)
              .then((doliUserId) => {
                const doliUSer = {
                  id: user.DoliID,
          ...
          //sino no se encuentra el id del usario de doli
          amoRepo.update(contactInfo);
        }
      }
    } else {
      ...
      amoRepo
        .insert(contactInfo)
        .then(() => {
          addContatToDoli(URLDoli, TOKENDoli, contactInfo, 3)
            .then((doliUserId) => {
              try {
                ...
                amoRepo
                  .updateDoliID(contactInfo.id, doliUserId)
                  .catch((e) => callback(e));
...
async function updateAmoContacts(numberRetries, callback) {
  ...
  try {
    ...
    amoconectionRepo.getAll().then((rows) => {
      if (rows.length > 0) {
        getContacts(rows[0].url, rows[0].accessToken, page)
          .then((data) => {
            ...
            amoRepo.deleteAll().then(() => {
              ...
              data.map((contact, index) => {
                ...
                let allTags = [];
                const amoLink =
                  "https://jmbere.amocrm.com/contacts/detail/" + contact.id;
              ...
              const dif = new Date() - start;
              logger.info(
                "Time to update amo contacts in miliseconds: " + dif.toString()
              );
...
function amoContactIsValid(contact) {
  if ((!contact.Email && !contact.Phone) || !isNaN(contact.name)) {
    ...
    contact.name.includes("Llamada") ||
    contact.name.length < 2 ||
    contact.name.includes("prueba") ||
    contact.name.includes("atencion amo") ||
    contact.name.includes("PROGRAMA DE") ||
    contact.name.includes("Google")
  ) {
    ...
    if (contact.Email.includes("noreply")) {
      ...
      contact.Email == "uber@uber.com" ||
      emailEnding == "rackear.com.ar" ||
      emailEnding == "blocket.com.ar" ||
      emailEnding == "sendinblue.com" ||
      emailEnding == "db-arg.com" ||
      emailEnding == "musitec.com.ar" ||
      emailEnding == "dbdrums.com.ar"
    ) {
      return false;
    } else {
      return true;
    ...
    if (contact.Phone) {
      if (contact.Phone.length < 5) return false;
    }
    return true;
  }
}
function contactIsPresent(target, contact) {
  if (
    ...
    comparePhones(target.Phone, contact.Phone)
  ) {
    return true;
  } else {
    return false;
...
async function getNewUsers(callBack) {
  ...
  amoRepo.getAll().then((amoData) => {
    ...
    var start1 = new Date();
    const filteredAmoData = amoData.filter((contact) => {
      return amoContactIsValid(contact);
    });
...
function filterDoliContacts(contact) {
  if (
    ...
    !isNaN(contact.name) ||
    contact.name.length < 2 ||
    contact.email == "jmbere@gmail.com" ||
    contact.email == "@"
  ) {
    //console.log("not email and not phone")
    return false;
    ...
    contact.name.includes("prueba") ||
    contact.name.includes("atencion amo") ||
    contact.name.includes("PROGRAMA") ||
    contact.name.includes("Google")
  ) {
    //console.log("filtrado por llamadas")
    return false;
  ...
  if (users.length == 0) {
    ...
    const filteredData = users.reduce((acc, current) => {
      ...
      const index = acc.findIndex((item) =>
        doliContactIsPresent(current, item)
      );
