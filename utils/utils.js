...
module.exports.exportExcel = function exportExcel(
  headers,
  data,
  fileName,
  callback
...
module.exports.filterContactData = function filterContactData(contact) {
  if (
    contact.name == "atencion amo crm" ||
    contact.name == "Uber" ||
    contact.Email == "calendar-notification@google.com" ||
    contact.Email == "googlemybusiness-noreply@google.com" ||
    contact.Email == "google-noreply@google.com" ||
    !isNaN(contact.name)
  ) {
    return false;
...
module.exports.convertDoliFormatToAmo = function convertDoliFormatToAmo(
  doliUser
) {
  const amoUser = {
    name: doliUser.name || "",
