...
const {
  ...
  getAmoUsers,
  deleteAmoConnection,
  getAllAmoConnections,
  insertAmoConmnection,
} = require("../model/DBupdater");

routes.get("/refreshtoken", requiresAuth(), async (req, res, next) => {
  try {
...
routes.post("/connections", requiresAuth(), (req, res, next) => {
  ...
  if (
    req.body.claveSecreta == "" ||
    req.body.url == "" ||
    req.body.code == "" ||
    req.body.id == ""
  ) {
    console.log("Here");
    res.status(200).render("amoconections", {
      conections: [],
      message: { type: "error", text: "Faltan datos!" },
    });
  } else {
    insertAmoConmnection(
      req.body.url,
      ...
      (err, newConect) => {
        if (err) {
          ...
          let data = [];
          data.push(newConect);
          res.render("amoconections", {
            conections: data,
            message: { type: "success", text: "Coneccion agregada con exito" },
          });
        }
      }
    );
