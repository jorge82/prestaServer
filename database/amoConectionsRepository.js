class AmoConectionRepository {
  ...
  createTable() {
    const sql =
      "CREATE TABLE IF NOT EXISTS amoconections (url TEXT PRIMARY KEY, clientId TEXT, clientSecret TEXT ,accessToken TEXT, refreshToken TEXT)";
    return this.dao.run(sql);
  }

  insert(conection) {
    console.log("inserting:", conection);
    const { url, clientId, clientSecret, accessToken, refreshToken } =
      conection;
    return this.dao.run(
      "INSERT INTO amoconections( url, clientId, clientSecret, accessToken, refreshToken) VALUES (?, ?, ?,?,?)",
      [url, clientId, clientSecret, accessToken, refreshToken]
    );
  }
  update(url, accessToken, refreshToken) {
    return this.dao.run(
      `UPDATE amoconections  SET accessToken = ?,refreshToken = ? WHERE url = ?`,
